clear;%close all;
k=linspace(0, 3*pi/1000,2e3);
n_r=1.5;n_i=0.03;c=1;
omega1=c*k;
oemga2=c*k/n_r;
[kk,omega]=meshgrid(k,c*k);
result=zeros(length(kk),length(kk));
...
% surf(x,y,z); view(0,90); %equivalent expression
shading interp; 
...
if num_EP>=3
    nx1_EP = locs(1); % ngular frequency
    nx2_EP = locs(2);
elseif num_EP==2
    nx1_EP = locs(1);
end
...
%% Poynting vector
figure(111)
Hx1 = -kk(nx_EP,ny_EP)/omega(nx_EP,ny_EP)*conj(Ey1);
Hx2 = -kk(nx_EP,ny_EP)/omega(nx_EP,ny_EP)*conj(Ey2);
Hx3 = -kk(nx_EP,ny_EP)/omega(nx_EP,ny_EP)*conj(Ey3);
Hx4 = -kk(nx_EP,ny_EP)/omega(nx_EP,ny_EP)*conj(Ey4);
Hx5 = -kk(nx_EP,ny_EP)/omega(nx_EP,ny_EP)*conj(Ey5);
