...
omega_list = linspace(1*c/d,3.5*c/d,1e2); %% jj

nb = 1.5; ns = 1; A = 0.1; delta =0.8; % index EP:0.99635
n1 = nb + A*(1-1i*delta);
n2 = nb - A*(1+1i*delta);
n3 = nb - A*(1-1i*delta);
n4 = nb + A*(1+1i*delta);
...
pcolor(beta/k_PBG,omega/(c/d),result');
shading interp;
xlabel('$k\ (k_{PBG})$','interpreter','latex','FontName','Times New Roman','FontSize',20)
ylabel('$\omega\ (c/d)$','interpreter','latex','FontSize',20)
...
abr = find(Phi_omega >1); % phase aberrant
for ii = 1:length(abr)  %  phase change
%     if abr(ii)<length(Phi)
        Phi_omega(abr(ii)) = (Phi(abr(ii)+1)-Phi(abr(ii))-2*pi)./(omega_PBG(2)-omega_PBG(1));
%     end
end
delta_t = - Phi_omega - nb*NN*d/c;
% delta_t = - Phi_omega;
plot(omega_PBG(1:end-1)/(c/d), delta_t)
